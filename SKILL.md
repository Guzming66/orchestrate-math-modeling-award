...
6. 当已有 v8/v9 工作区时运行 `python scripts/migrate_workspace.py <workspace>`；迁移保留旧文件，并把模型证据、审查路由和论文载荷升级为 v10 schema。
...
- 模型冻结后生成 schema v1 `synthesis/paper_payload.json`；论文手以它为主要科学输入，只在核对事实时读取结果或引用台账。
...
硬阻断仅限会使结论、复现、表达、合规或提交失效的问题：未核验规则、缺少 provenance、核心结果不可复现、Evidence Profile/Review Router 不完整、数学定义与实现不一致、模型选择无理由、创新或论文强主张无证据、Paper Payload/表达防火墙失败、未关闭 critical/超阈值 major、哈希失配、profile 要求的产物缺失、匿名/凭据问题或 LaTeX 构建失败。探索宽度、模型数量和复杂度不属于硬门禁。
